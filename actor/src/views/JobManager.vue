...
const columns = [
  ...
  {title: 'run', key: 'openRun', ellipsis: true},
  {title: 'status', key: 'status', ellipsis: true},
  {
    title: 'opt', key: 'opt', ellipsis: true, render(row: any) {
      return [h(
          NButton,
          {strong: true, tertiary: true, type:"error",size: "small", onClick: () => stopJob(row.name).then(r => getData())},
          {default: () => "停止"}
      ), h(
          NButton,
          {strong: true, tertiary: true, type:"primary",size: "small", onClick: () => runJob(row.name).then(r => getData())},
          {default: () => "run"}
      )
      ]
    }
